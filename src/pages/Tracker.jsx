...
const Tracker = () => {
  ...
  const fetchTrackerData = async () => {
    try {
      ...
      if (!isAuthenticated || !currentUser) {
        console.log("Not authenticated or currentUser not available. Skipping data fetch.");
        setIsLoading(false);
        return;
      }
      ...
      console.log("Columns targeted for display (0-indexed):", currentColumnsToDisplay);
      ...
      if (fmsData.table.cols) {
        fmsData.table.cols.forEach((col, index) => {
          ...
          if (currentColumnsToDisplay.includes(index)) {
            let label = col.label || `Column ${String.fromCharCode(65 + index)}`;
        ...
        return dataRows.map((row, rowIndex) => {
          ...
          if (row.c) {
            row.c.forEach((cell, i) => {
              ...
              if (currentColumnsToDisplay.includes(i) || i === 14 || i === 15 || i === 17 || i === SALES_PERSON_COLUMN_INDEX) {
      ...
      console.log("Raw FMS Items (after processRows, before further filtering):", fmsItems);
      ...
      let filteredFMSItems = fmsItems.filter((item) => {
        ...
        const hasContentInDisplayedColumns = currentColumnsToDisplay.some((colIndex) => {
      ...
      if (userRole.toLowerCase() !== "admin") {
        filteredFMSItems = filteredFMSItems.filter(item => {
          const salesPersonInRow = String(item[`col${SALES_PERSON_COLUMN_INDEX}`] || "").toLowerCase();
      ...
      console.log("Final Filtered FMS Items (to be displayed):", filteredFMSItems);
  ...
  const exportData = () => {
    try {
      const exportHeaders = [{ id: "action", label: "Action" }, ...sheetHeaders];
      ...
      const csvContent = [
        ...
        ...filteredIndents.map((item) => {
          ...
          const rowValues = sheetHeaders
            .map((header) => {
              let value = item[header.id] || "";
              // Apply date formatting for export as well
              if (header.id === 'col12' || header.id === 'col13') { // Assuming these are date columns
                        ...
                        >
                          ...
                          {sheetHeaders.map((header) => (
                            ...
                            >
                              ...
                              {(header.id === 'col12' || header.id === 'col13' || header.id === 'col17') // Added col17 for Last Call Date
