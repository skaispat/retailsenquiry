  ...
  const fetchMasterDataForDealerSizes = async () => {
    ...
    try {
      ...
      if (data.table?.rows?.length > 0) {
        dealerSizes = data.table.rows.map((row) => {
          ...
          return size &&
            ...
            !lowerCaseSize.startsWith("date("); // Filter out "Date(year,month,day,...)" strings
        });
      }
      
      const uniqueDealerSizes = Array.from(new Set(dealerSizes)); // Ensure unique sizes
      setFetchedDealerSizes(uniqueDealerSizes.length > 0 ? uniqueDealerSizes : DEFAULT_DEALER_SIZES);
    } catch (err) {
          ...
          <div className=" p-2 lg:p-8">
            <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    {userRole === "Admin" ? (
                      <div className="relative" ref={salesPersonDropdownRef}>
                        <button
                          ...
                          className={`w-full px-4 py-3 text-left bg-white border border-slate-200 rounded-xl shadow-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all duration-200 text-slate-700 font-medium flex justify-between items-center ${isLoadingSalesPersons ? "opacity-70 cursor-not-allowed" : ""
                            }`}
                      ...
                      <input
                        ...
                        className={`w-full px-4 py-3 bg-white border border-slate-200 rounded-xl shadow-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all duration-200 text-slate-700 font-medium ${userRole === "User" ? "bg-gray-50 cursor-not-allowed" : ""
                          }`}
                      />
                    )}
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid gap-6 md:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      >
                        <option value="">
                          {isLoadingDealerSizes ? "Loading dealer sizes..." : "Select dealer size"}
                        </option>
                      ...
                      {errorDealerSizes && (
                        <p className="text-red-500 text-sm mt-2 font-medium">{errorDealerSizes}</p>
                      )}
                    </div>
              ...
              <div className="pt-6 border-t border-slate-200">
                <button
                  type="submit"
                  disabled={isSubmitting || (userRole === "Admin" && isLoadingSalesPersons) || isLoadingDealerSizes}
                  className={`w-full lg:w-auto bg-gradient-to-r from-purple-600 via-blue-600 to-indigo-600 hover:from-purple-700 hover:via-blue-700 hover:to-indigo-700 text-white font-bold py-4 px-8 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-[1.02] ${isSubmitting || (userRole === "Admin" && isLoadingSalesPersons) || isLoadingDealerSizes
        ...
        <div className="fixed inset-0 bg-black bg-opacity-10 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full mx-4 transform transition-all duration-300 scale-100">
            <div className="p-8 text-center">
              ...
              {/* Title */}
              <h2 className={`text-2xl font-bold mb-4 ${modalType === "success" ? "text-green-600" : "text-red-600"}`}>
