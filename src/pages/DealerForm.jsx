...
function DealerForm() {
  ...
  /**
   * Dynamically determines the minimum number of digits based on the next number.
   * @param {number} num - The number to format
   * @returns {string} - The formatted dealer code
   */
  const formatDealerCode = (num) => {
    // Determine minimum digits based on the number
    let minDigits = 2; // Start with minimum 2 digits
    if (num >= 100) minDigits = 3;
    if (num >= 1000) minDigits = 4;
    if (num >= 10000) minDigits = 5;
    if (num >= 100000) minDigits = 6;
    // Add more conditions as needed for larger numbers
    
    return `DC${num.toString().padStart(minDigits, "0")}`;
  }
   ...
   * The code format supports unlimited digits (e.g., DC01, DC02, ..., DC99, DC100, DC1000, etc.).
   */
  const generateNextDealerCode = async () => {
    ...
    try {
      ...
      // Google-visualisation SQL to fetch ALL dealer codes from col B
      ...
      // Extract all valid dealer codes and find the maximum number
      let maxNumber = 0
      
      rows.forEach(row => {
        if (row.c && row.c[0] && row.c[0].v) {
          const code = row.c[0].v.toString().trim().toUpperCase()
          const match = code.match(/^DC(\d+)$/i)
          if (match) {
      ...
      // Generate next dealer code with dynamic formatting
      const nextNumber = maxNumber + 1
      const nextDealerCode = formatDealerCode(nextNumber)
